...
void virtio_blk_init(void) {
    ...
    // 2. Set the ACKNOWLEDGE status bit: Device has been found
    ...
    // 5. Initialize the virtqueue for disk read/write requests
...
void read_write_disk(void *buf, unsigned sector, int is_write) {
    if (sector >= blk_capacity / SECTOR_SIZE) {
        printf("virtio: tried to read/write secto=%d, but capacity is %d\n",
...
void boot(void) {
    __asm__ __volatile__(
        ...
        "j kernel_main\n"       // Jump to kernel_main
        :
        : [stack_top] "r" (__stack_top) // Pass the stack top address as an input(%[stack_top])
    );
}
